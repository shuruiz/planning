...
while episode_count<2000000:  # Run until solved
	...
	if episode_count%100 ==0:
		print("model8a_thru episode %d running reward %f" %(episode_count, running_reward))
	if episode_count%5000==0:
		np.save('model8a_thru_episode_history', episode_reward_history)
		print("reward history saved")
		try:
			model.save('reduced_model8a_thru') # only one task
			model_target.save('reduced_target_model8a_thru')
