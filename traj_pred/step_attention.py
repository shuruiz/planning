...
class AttentionAugmentation2D(Layer):
    ...
    def __init__(self,depth_k=0.2, depth_v=0.2, \
        num_heads=4, relative=True, **kwargs):
        """
    ...
    def call(self, inputs, **kwargs):
        ...
        q, k, v = tf.split(inputs, [self.depth_k, self.depth_k, self.depth_v], axis=-1) # get q, k ,v based on convolutions and split them into query, k and value 
        ...
        # print("logits", K.print_tensor(logits))

        # Apply relative encodings
        if self.relative:
            h_rel_logits, w_rel_logits = self.relative_logits(q)
            # print("h_rel_logits",h_rel_logits)

            # print("w_rel_logits",w_rel_logits)
            logits += h_rel_logits
            logits += w_rel_logits
    ...
    def rel_to_abs(self, x):
        ...
        # print("rel_to_abs",shape)
        # print('B', B)
        # print('Nh', Nh)
        # print('L', L)
        # print('stack', K.stack([B, Nh, L, 1]))
